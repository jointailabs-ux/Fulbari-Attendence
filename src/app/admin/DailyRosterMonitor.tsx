...
const STATUS_MAP = {
  ...
  NOT_STARTED: {
    label: "Not Started / Absent",
    color: "#94a3b8",
    bg: "rgba(148, 163, 184, 0.08)",
    icon: "⚪",
    auditColor: "var(--brand-secondary)",
    auditLabel: "ABSENT",
  },
};
    ...
    >
      <div
        style={{
          ...
          padding: "2rem",
          boxShadow:
            "0 24px 60px rgba(139,92,246,0.2), 0 8px 32px rgba(0,0,0,0.6)",
          position: "relative",
        ...
        {success ? (
          ...
          >
            ...
            <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              The salary advance has been recorded.
            </p>
            <button
              onClick={onClose}
              style={{
            ...
            <div style={{ display: "flex", gap: "0.75rem" }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: "white",
                  cursor:
                    submitting || !amount ? "not-allowed" : "pointer",
                  fontWeight: 800,
                  fontSize: "0.9rem",
...
// ─── Activity Modal (same style as AttendanceTab audit popup) ─────────────────
function ActivityModal({
  ...
  const badge = STATUS_MAP[staff.state] || STATUS_MAP.NOT_STARTED;
  const hasActivity = staff.state !== "NOT_STARTED" && staff.startTime;

  // Compute work hours from the data already in the roster item
  ...
  if (staff.startTime) {
    ...
    let totalBreakMs = 0;
    staff.breaks.forEach((b) => {
      const bs = new Date(b.startTime).getTime();
      const be = b.endTime ? new Date(b.endTime).getTime() : Date.now();
      totalBreakMs += be - bs;
    });
    const netMs = Math.max(0, end - start - totalBreakMs);
    workHoursDisplay = (netMs / 3600000).toFixed(2);
  }
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              {staff.name} — Security and deployment status.
            </p>
          </div>
          ...
          {hasActivity ? (
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontWeight: 800, color: badge.auditColor }}
                >
                  {badge.auditLabel}
                </span>
              </div>
            ...
            >
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
                No activity logs for this cycle.
              </p>
            </div>
          )}
        </div>
...
export default function DailyRosterMonitor({ roster }: { roster: RosterItem[] }) {
  const [filter, setFilter] = useState<
    "ALL" | "PRESENT" | "BREAK" | "ENDED" | "ABSENT"
  >("ALL");
  ...
  useEffect(() => {
    ...
    const todayStr = new Date(localNow.getTime() - tzOffset)
      .toISOString()
      .split("T")[0];
  ...
  const filteredRoster = useMemo(() => {
    return currentRoster.filter((item) => {
      ...
      if (selectedOutlet !== "ALL" && item.location !== selectedOutlet)
        return false;
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
              No personnel found for the selected search & filter criteria.
            </p>
          </div>
        ) : (
          <div
          ...
          >
            {filteredRoster.map((item, idx) => {
              ...
              const hasActivity =
                item.state !== "NOT_STARTED" && !!item.startTime;
                ...
                >
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontFamily: "monospace",
                          color:
                            item.breakTimeStr && item.breakTimeStr !== "--"
                              ? "#f59e0b"
                              : "var(--text-muted)",
                        }}
                      >
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontFamily: "monospace",
                          color:
                            item.workTimeStr && item.workTimeStr !== "--"
                              ? "var(--brand-accent)"
                              : "var(--text-muted)",
                        }}
                      >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        hasActivity ? setActivityModal(item) : undefined
                      }
                      disabled={!hasActivity}
                      title={
                        !hasActivity
                          ? "No activity recorded for this staff today"
                      ...
                      onMouseEnter={(e) => {
                        ...
                        e.currentTarget.style.background =
                          "linear-gradient(135deg, rgba(6,182,212,0.28), rgba(59,130,246,0.28))";
                        e.currentTarget.style.borderColor =
                          "rgba(6,182,212,0.5)";
                      }}
                      onMouseLeave={(e) => {
                    ...
                    <Link
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = `linear-gradient(135deg, ${c1}25, ${c2}25)`;
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = `linear-gradient(135deg, ${c1}15, ${c2}15)`;
                      }}
                    >
                      👤 Profile
                    </Link>
